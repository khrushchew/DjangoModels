...
# UPD2: Добавлен постраничный вывод на /news/ - на одной странице было не больше 10 новостей и видны номера лишь ближайших страниц, а также имеется возможность перехода к первой или последней странице.
Добавлена страница /news/search. На ней должна реализована возможность искать новости по определённым критериям. Критерии должны следующие:
1) По названию
2) По имени автора
3) Позже указываемой даты (Вводится в формате указанном в статье - 28 Mar 2024)<br>
# Появились страницы создания, редактирования и удаления новостей и статей, а также обработчики ошибок через 404-error:
1) /news/create/
2) /news/<int:pk>/edit/
3) /news/<int:pk>/delete/
4) /articles/create/
5) /articles/<int:pk>/edit/
6) /articles/<int:pk>/delete/


# UPD: Теперь есть страница с новостями по пути /news, а также фильтр нежелательных слов
